...
function watchDirectory (bool) {
	if (bool) {
		...
		gaze = null;
		gaze = new Gaze('*.ls', {cwd: options.watch.directory}, function (err, watcher) {
			this.on('changed', function (filepath) {
				console.log(filepath + ' was changed');
				var dest = getDestination(filepath);
				start = +new Date();
				readLiveFile(filepath, dest, compileLive);
			});
...
holder.ondrop = function (e) {
	...
	if ( files.length === 1 ) {
		...
		if ( extension === '.ls' ) {
			readLiveFile(file.path, dest, compileLive);
		} else {
		...
		_.forEach(files, function(n, key) {
			if ( path.extname(files[key].path) === '.ls' ) {
				liveFiles.push(files[key].path);
			}
		});
		readManyLiveFiles(liveFiles, dest, compileLive);
	};
	return false;
};
...
function compileLive (liveStr, fileName, dest , many) {
	var js = LiveScript.compile(liveStr);
...
function readLiveFile (src, dest, callback) {
	var fileName = path.basename(src, '.ls');
...
function readManyLiveFiles (array, dest, callback) {
	array.forEach(function (filePath) {
		var fileName = path.basename(filePath, '.ls');
